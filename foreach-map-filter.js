...
function onlyEvenValues(arr) {
  const evenValuesArr = [];
  arr.forEach(function (value) {
    if (value % 2 === 0) evenValuesArr.push(value);
  });
...
function showFirstAndLast(arr) {
  ...
  arr.forEach(function (value) {
    let newName = "";
    newName += value[0];
    newName += value[value.length - 1];
    firstAndLastArr.push(newName);
  });
...
function vowelCount(str) {
  ...
  Array.from(str.toLowerCase()).forEach(function (letter) {
    if (letter === "a") {
      newObj.a ? (newObj.a += 1) : (newObj.a = 1);
    } else if (letter === "e") {
      newObj.e ? (newObj.e += 1) : (newObj.e = 1);
    } else if (letter === "i") {
      newObj.i ? (newObj.i += 1) : (newObj.i = 1);
    } else if (letter === "o") {
      newObj.o ? (newObj.o += 1) : (newObj.o = 1);
    } else if (letter === "u") {
      newObj.u ? (newObj.u += 1) : (newObj.u = 1);
    }
  });
  return newObj;
}
